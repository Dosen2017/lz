...
//专门用于过审的接口地址
class IosLogin extends Controller
{
    const SUCCESS = 0;
    const FAIL = 1;
    const FAIL2 = 2;
    ...
    public function initApp()
    {
        ...
        if (empty($gameId) || empty($time) || empty($sign)) {
            $this->echoJson(self::FAIL);
        }

        $url = $this->urlArr[$gameId];
        if (empty($url)) {
            $this->echoJson(self::FAIL);
        }

        $this->echoJson(self::SUCCESS, ['is_open' => 1, 'is_active' => 1]);
    }
    ...
    public function register()
    {
        ...
        if (empty($gameId) || empty($userName) || empty($password) || empty($time) || empty($sign)) {
            $this->echoJson(self::FAIL);
        }
...
//        $url = '';
        ...
        $this->echoJson(self::SUCCESS, ['token' => '']);
    }
...
//        }
//
//        $this->echoJson(self::SUCCESS, ['token' => 'https://www.baidu.com/']);
    }
...
(b) 在使用本应用网络服务进行网上交易时，您不可避免的要向交易对方或潜在的交易对手透露信息。<br/>
<br/>
7.本隐私政策的更改<br/>
...
EOD;
    ...
    {
        echo "
网站&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;&nbsp;http://www.lezhonggame.com<br/>
Email&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;&nbsp;chaofanwan@163.com<br/>
客服热线&nbsp;:&nbsp;&nbsp;(020)82259854<br/>
";
    exit;
    }

    protected function echoJson($ret, $msg = null, $exit = true, $isJson = true)
    {
