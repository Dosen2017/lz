 ...
 * @icon fa fa-circle-o
 */
class RetentionPay extends Backend
{
    
    /**
     * RetentionPay模型对象
     * @var \app\admin\model\datastat\RetentionPay
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\datastat\RetentionPay;
    ...
    public function getList(&$total)
    {
        ...
        foreach ($dateList as $k => $v) {
            ...
            $regSql = "select count(1) as register_num from game_users where create_d = " . intval($v) . $where;
            $regData = $this->model->query($regSql);

            $sql = "select count(1) as s_pay_num, 
                                ...
                                count(active60=1 or null) as s_active60
                                  from game_users where charge1 = 1 and create_d = " . intval($v) . $where;  //第一天充值的玩家
            $statKeepData = $this->model->query($sql);
            $list[$k]['register_num'] = $regData[0]['register_num'];
            $list[$k]['s_pay_num'] = $statKeepData[0]['s_pay_num'];
