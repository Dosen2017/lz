...
use app\admin\model\channelpack\ChannelPack;
...
class Delivery extends Backend
{
    ...
    /**
     * Delivery模型对象
     * @var \app\admin\model\datastat\Delivery
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\datastat\Delivery;
...
//var_dump($list);exit;
    ...
    public function getList2(&$total) {
        ...
        $this->conditionHandle2(true, 'complete_time', $where, $bIdList);

        foreach ($bIdList as $k => $v) {
            $list[$k]['b_id'] = $v;

            //注册数量
            $sql = "select count(1) as register_num from game_users where b_id = '" . $v  . "'" . str_replace("complete_time", "create_time", $where);
//            echo $sql . "\n";
            $regData = $this->model->query($sql);
            $list[$k]['register_num'] = $regData[0]['register_num'];  //注册数量
//var_dump($list);exit;
            ...
            $sqlAll = "select  sum(amount) as pay_money, 
                                count(distinct user_num) as pay_person_num 
                                from game_orders where b_id = '" . $v . "'" . $where;
//            echo $sqlAll . "\n";
            $payDataAll = $this->model->query($sqlAll);
            $list[$k]['pay_money'] = round($payDataAll[0]['pay_money'] / 100, 2);     //充值金额
            $list[$k]['pay_person_num'] = $payDataAll[0]['pay_person_num'];//充值人数
            ...
            //次留数
            $sql = "select count(active2=1 or null) as s_active2
                                  from game_users where b_id = '" . $v . "'" . str_replace("complete_time", "create_time", $where);
//            echo $sql . "\n";
            $statKeepData = $this->model->query($sql);
            $list[$k]['s_active2'] = $statKeepData[0]['s_active2'];
            $list[$k]['s_active2_per'] = $this->statKeepShow($statKeepData[0]['s_active2'],  $list[$k]['register_num']);
...
//            echo $sql . "\n";
            ...
            $sqlDau = "select count(distinct user_num) as dau from user_logs where b_id = '" . $v . "'" . str_replace("complete_time", "opt_time", $where);
            $dauData = $this->model->query($sqlDau);
    ...
    public function conditionHandle2($isOpt = false, $timeField = 'time', &$where = '', &$bIdList = []) {
        if ($isOpt) {
            ...
            $bIdList = $this->getBidsList($filterData);
        }

    }

    public function getBidsList($filterData) {

        $channelPackM = new ChannelPack();
        $cpData = $channelPackM->getKVList();
        if (!isset($filterData['b_id'])) {
            return array_values($cpData);
        } else {

            $bIds = array_intersect($filterData['b_id'], array_keys($cpData));  //得到的分包的数字ID
            rsort($bIds);
            $qBId = [];
            foreach ($bIds as $v) {
                if (0 == $v)
