...
trait Backend
{
    ...
    public function conditionHandle($isOpt = false, $timeField = 'time', &$where = '', &$dateList = []) {
        if ($isOpt) {
            $startTime = $endTime = 0;
            $where = "";
            ...
            if (!empty($filterData)) {
                ...
                //统一整理
                $commonConditions = ['game_num', 'os_type', 'channel_num', 'channel_pkg_num', 'b_id', 'ads_id'];
...
//echo $where;exit;
    ...
    protected function renderMultiSelect()
    {
        ...
        $channelPkgM = new ChannelPkg();
        $userPagesM = new UserPages();
        $channelPackM = new ChannelPack();

        $this->assignconfig("gameList", json_encode($gameM->getKVList()));
        $this->assignconfig("channelList", json_encode($channelM->getKVList()));
        $this->assignconfig("channelPkgNumList", json_encode($channelPkgM->getKVList()));
        $this->assignconfig("adsIdsKVList", json_encode($userPagesM->getKVList()));
        $this->assignconfig("adsIdsList", json_encode($userPagesM->getVVList()));

        $this->assignconfig("bIdList", json_encode($channelPackM->getKVList()));
    }
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                $this->beforeAdd($params);  //类似函数。不能再try里面，会导致错误提示信息为空的问题，。暂时还不知原因
...
//        echo json_encode($row);exit;
        $this->renderEdit($row);  //编辑的时候，解析json字符串渲染一下
